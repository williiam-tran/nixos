{
  ...
  inputs = {
    ...
    nixpkgs-linux-zen_6_12_2.url = "github:NixOS/nixpkgs/882842d2a908700540d206baa79efb922ac1c33d";
  };
  ...
  outputs = {
    ...
    prisma-utils,
    alejandra,
    nixpkgs-linux-zen_6_12_2,
    ...
  } @ inputs: let
    system = "x86_64-linux";

    # Configured pkgs for the zen kernel
    pkgs_zen_kernel = import inputs.nixpkgs-linux-zen_6_12_2 {
      inherit system;
      config = {
        allowUnfree = true;
      };
  ...
  in {
    ...
    nixosConfigurations.william = nixpkgs.lib.nixosSystem {
      ...
      specialArgs = {
        inherit inputs;
        pkgs_zen_kernel = pkgs_zen_kernel;
      };
      modules = [
        (
          {
